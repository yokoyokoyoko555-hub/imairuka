...
story += [
    ...
    Spacer(1, 12 * mm),
    p("ASP / SaaS版 現時点版<br/>対象: 利用企業の管理者・担当者 / 更新日: 2026年5月26日", "CoverMeta"),
    Spacer(1, 8 * mm),
...
story += [
    ...
    table(
        [
            ...
            ["ユーザー管理", "アカウント招待、権限変更、停止、標準1名までのユーザー枠表示"],
        ],
        [38 * mm, 132 * mm],
    ),
    p("2. 利用開始の流れ", "H2"),
...
story += [
    ...
    *numbered(
    ...
    note("OTPログインが有効な場合は、メールアドレスとパスワード入力後に登録メールアドレスへ届く認証コードを入力します。", "#eff6ff", "#2563eb"),
    p("案件詳細で確認できること", "H3"),
...
story += [
    ...
    *numbered(
    ...
    p("9. ユーザー管理と権限", "H2"),
    p("管理者権限を持つユーザーは、同じ会社のユーザーを招待できます。基本プランは全機能利用可能で、標準1名まで利用できます。"),
    table(
        [
            ["権限", "概要"],
    ...
    note("追加アカウントは1名ごとに別途課金が必要です。未課金または上限到達時は招待できません。", "#fffbeb", "#f59e0b"),
    PageBreak(),
]
...
story += [
    ...
    *numbered(
    ...
    table(
        [
            ["状況", "確認ポイント"],
            ["ログインできない", "メールアドレス、パスワード、招待URLの有効期限を確認してください。OTP有効化後は認証コードの期限も確認してください。"],
            ["ユーザーを招待できない", "契約状態、標準ユーザー枠、既存の招待中メールアドレスを確認してください。追加アカウントは別途課金が必要です。"],
    ...
    Spacer(1, 8 * mm),
    p("最終更新: 2026年5月26日 / Imairuka ASP版 利用者マニュアル", "SmallJP"),
]
