...
class QuotationForm {
  ...
  async loadProducts(query = '') {
    try {
      const response = await fetch(`/quotations/search_products?q=${encodeURIComponent(query)}`);
  ...
  attachAddItemListeners() {
    document.querySelectorAll('.add-item').forEach(button => {
      button.addEventListener('click', () => this.addProductToQuotation(button));
    });
  }
  ...
  addProductToQuotation(button) {
    ...
    row.innerHTML = `
      <td><input type="text" name="quotation[quotation_items_attributes][${index}][product_code]" value="${this.escapeHtml(productCode)}" class="form-control" required></td>
      <td><input type="text" name="quotation[quotation_items_attributes][${index}][product_name]" value="${this.escapeHtml(productName)}" class="form-control" required></td>
      <td class="text-end"><input type="number" name="quotation[quotation_items_attributes][${index}][unit_price]" value="${unitPrice}" class="form-control text-end" min="0" step="1" required></td>
      <td class="text-end"><input type="number" name="quotation[quotation_items_attributes][${index}][quantity]" value="1" class="form-control text-end" min="1" step="1" required></td>
      <td class="text-end"><span class="item-amount">¥${unitPrice.toLocaleString()}</span></td>
      <td><select name="quotation[quotation_items_attributes][${index}][tax_rate]" class="form-select">
  ...
  reindexRows() {
    ...
    rows.forEach((row, index) => {
      const inputs = row.querySelectorAll('input[name*="quotation_items_attributes"]');
      inputs.forEach(input => {
        const name = input.name;
        const newName = name.replace(/quotation_items_attributes\[\d+\]/, `quotation_items_attributes[${index}]`);
        input.name = newName;
        
        const id = input.id;
        if (id) {
          const newId = id.replace(/quotation_items_attributes_\d+/, `quotation_items_attributes_${index}`);
          input.id = newId;
        }
      });
    });
  ...
  updateTotalAmount() {
    ...
    document.querySelectorAll('#quotation-items tr.item-row:not([style*="display: none"])').forEach(row => {
      const unitPrice = parseInt(row.querySelector('input[name*="[unit_price]"]').value) || 0;
      const quantity = parseInt(row.querySelector('input[name*="[quantity]"]').value) || 0;
    ...
    // hidden fieldも更新
    const hiddenTotalField = document.querySelector('input[name="quotation[total_amount]"]');
    if (hiddenTotalField) {
  ...
  openPreview() {
    // 正しいフォームを選択（見積書フォーム）
    const form = document.querySelector('form[action*="/quotations"]');
    if (!form) {
      console.error('見積書フォームが見つかりません');
      return;
    }
    
    const formData = new FormData(form);
    
    const subject = formData.get('quotation[subject]');
    
    const quotationData = {
      quotation_number: formData.get('quotation[quotation_number]') || '',
      quotation_date: formData.get('quotation[quotation_date]') || '',
      customer_name: formData.get('quotation[customer_name]') || '',
      customer_address: formData.get('quotation[customer_address]') || '',
      subject: subject || '',
      staff_name: formData.get('quotation[staff_name]') || '',
      valid_until: formData.get('quotation[valid_until]') || '',
      status: formData.get('quotation[status]') || '',
      notes: formData.get('quotation[notes]') || '',
      total_amount: 0,
      items: []
    };

    document.querySelectorAll('#quotation-items tr.item-row').forEach(row => {
      ...
      if (productCode && productName) {
        ...
        quotationData.total_amount += amount;
      }
    });

    const previewUrl = '/quotations/print?' + new URLSearchParams({
  ...
  downloadPDF() {
    ...
    const pdfUrl = '/quotations/download_pdf?' + new URLSearchParams({
      quotation_data: JSON.stringify(quotationData)
    });
  ...
  bindEvents() {
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        ...
        const hiddenRows = document.querySelectorAll('#quotation-items tr.item-row[style*="display: none"]');
