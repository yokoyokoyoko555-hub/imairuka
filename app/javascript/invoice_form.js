...
class InvoiceForm {
  ...
  async loadProducts(query = '') {
    try {
      const response = await fetch(`/invoices/search_products?q=${encodeURIComponent(query)}`);
  ...
  attachAddItemListeners() {
    document.querySelectorAll('.add-item').forEach(button => {
      button.addEventListener('click', () => this.addProductToInvoice(button));
    });
  }

  // 請求書に商品を追加
  addProductToInvoice(button) {
    ...
    row.innerHTML = `
      <td><input type="text" name="invoice[invoice_items_attributes][${index}][product_code]" value="${this.escapeHtml(productCode)}" class="form-control" required></td>
      <td><input type="text" name="invoice[invoice_items_attributes][${index}][product_name]" value="${this.escapeHtml(productName)}" class="form-control" required></td>
      <td class="text-end"><input type="number" name="invoice[invoice_items_attributes][${index}][unit_price]" value="${unitPrice}" class="form-control text-end" min="0" step="1" required></td>
      <td class="text-end"><input type="number" name="invoice[invoice_items_attributes][${index}][quantity]" value="1" class="form-control text-end" min="1" step="1" required></td>
      <td class="text-end"><span class="item-amount">¥${unitPrice.toLocaleString()}</span></td>
      <td><select name="invoice[invoice_items_attributes][${index}][tax_rate]" class="form-select">
  ...
  reindexRows() {
    ...
    rows.forEach((row, index) => {
      const inputs = row.querySelectorAll('input[name*="invoice_items_attributes"]');
      inputs.forEach(input => {
        const name = input.name;
        const newName = name.replace(/invoice_items_attributes\[\d+\]/, `invoice_items_attributes[${index}]`);
        input.name = newName;
        
        const id = input.id;
        if (id) {
          const newId = id.replace(/invoice_items_attributes_\d+/, `invoice_items_attributes_${index}`);
          input.id = newId;
        }
      });
    });
  ...
  updateTotalAmount() {
    ...
    document.querySelectorAll('#invoice-items tr.item-row:not([style*="display: none"])').forEach(row => {
      const unitPrice = parseInt(row.querySelector('input[name*="[unit_price]"]').value) || 0;
      const quantity = parseInt(row.querySelector('input[name*="[quantity]"]').value) || 0;
    ...
    // hidden fieldも更新
    const hiddenTotalField = document.querySelector('input[name="invoice[total_amount]"]');
    if (hiddenTotalField) {
  ...
  openPreview() {
    ...
    if (!form) {
      console.error('請求書フォームが見つかりません');
      return;
    }
    
    const formData = new FormData(form);
    
    const paymentMethod = formData.get('invoice[payment_method]');
    const subject = formData.get('invoice[subject]');
    ...
    const invoiceData = {
      ...
      subject: subject || '',
      staff_name: formData.get('invoice[staff_name]') || '',
      payment_due_date: formData.get('invoice[payment_due_date]') || '',
    ...
    document.querySelectorAll('#invoice-items tr.item-row').forEach(row => {
    ...
    const previewUrl = '/invoices/print?' + new URLSearchParams({
  ...
  downloadPDF() {
    ...
    const pdfUrl = '/invoices/download_pdf?' + new URLSearchParams({
      invoice_data: JSON.stringify(invoiceData)
    });
  ...
  bindEvents() {
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        ...
        const hiddenRows = document.querySelectorAll('#invoice-items tr.item-row[style*="display: none"]');
