class Quotation < ApplicationRecord
  ...
  validates :quotation_number, uniqueness: { allow_blank: true }, unless: :draft?
  validates :quotation_date, presence: true, unless: :draft?
  ...
  before_create :set_quotation_number
  ...
  def self.ransackable_attributes(auth_object = nil)
    ["created_at", "customer_name", "discarded_at", "id", "quotation_date", "quotation_number", "staff_name", "status", "total_amount", "notes", "updated_at"]
  end

  # Ransackで検索可能なアソシエーションを定義
  def self.ransackable_associations(auth_object = nil)
    ["quotation_items", "quotation_histories"]
  end
  ...
  def set_quotation_number
    return if quotation_number.present?
    
    # 最新の見積書番号を取得（論理削除されていないもの）
    last_quotation = Quotation.kept.order(quotation_number: :desc).first
    
    if last_quotation && last_quotation.quotation_number.match?(/^Q\d{6}$/)
      # 既存の番号から連番を抽出して+1
      last_number = last_quotation.quotation_number[1..-1].to_i
    ...
    # 重複しない番号を見つけるまでループ
    loop do
      candidate_number = "Q#{format('%06d', next_number)}"
      unless Quotation.kept.exists?(quotation_number: candidate_number)
        self.quotation_number = candidate_number
        break
      end
      next_number += 1
    end
  ...
  def calculate_total_amount
    self.total_amount = quotation_items.reject(&:marked_for_destruction?).sum { |item| item.amount.to_i }
  end

  def record_creation_history
    action_text = draft? ? '一時保存' : '見積書作成'
    quotation_histories.create!(
      action: action_text
    )
  end

  def record_update_history
    action_text = draft? ? '一時保存' : '見積書更新'
    quotation_histories.create!(
      action: action_text
    )
  end
  ...
  def must_have_at_least_one_item
    if quotation_items.reject(&:marked_for_destruction?).empty?
      errors.add(:base, '商品を1つ以上追加してください')
    end
  end

  def no_items?
    quotation_items.reject(&:marked_for_destruction?).empty?
  end
end
