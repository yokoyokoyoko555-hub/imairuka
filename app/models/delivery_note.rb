class DeliveryNote < ApplicationRecord
  ...
  has_many :delivery_note_histories, dependent: :destroy
  ...
  accepts_nested_attributes_for :delivery_note_items, allow_destroy: true, reject_if: :all_blank
  ...
  def self.ransackable_attributes(auth_object = nil)
    ["created_at", "customer_name", "discarded_at", "id", "delivery_date", "delivery_number", "staff_name", "status", "total_amount", "notes", "updated_at"]
  end

  # Ransackで検索可能なアソシエーションを定義
  def self.ransackable_associations(auth_object = nil)
    ["delivery_note_items", "delivery_note_histories"]
  end
  ...
  def set_delivery_number
    ...
    last_note = DeliveryNote.kept.order(delivery_number: :desc).first
    if last_note && last_note.delivery_number.match?(/^D\d{6}$/)
      last_number = last_note.delivery_number[1..-1].to_i
    ...
    # 重複しない番号を見つけるまでループ
    loop do
      candidate_number = "D#{format('%06d', next_number)}"
      unless DeliveryNote.kept.exists?(delivery_number: candidate_number)
        self.delivery_number = candidate_number
        break
      end
      next_number += 1
    end
  ...
  def calculate_total_amount
    self.total_amount = delivery_note_items.reject(&:marked_for_destruction?).sum(&:amount)
  end

  def must_have_at_least_one_item
    if delivery_note_items.reject(&:marked_for_destruction?).empty?
      errors.add(:base, '商品を1つ以上追加してください')
    end
  end

  def no_items?
    delivery_note_items.reject(&:marked_for_destruction?).empty?
  end

  def record_creation_history
    action_text = draft? ? '一時保存' : '納品書作成'
    delivery_note_histories.create!(
      action: action_text
    )
  end

  def record_update_history
    action_text = draft? ? '一時保存' : '納品書更新'
    delivery_note_histories.create!(
      action: action_text
    )
  end
